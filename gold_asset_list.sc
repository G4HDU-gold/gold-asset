...
if($sql->db_Select_gen($gasset_arg, false))
{
	...
	while ($gasset_row = $sql->db_Fetch())
	{
	    ...
	    if(!empty($gasset_asset) && is_readable(e_PLUGIN.'gold_asset/assets/'.$gasset_asset.'/asset.php') )
		{
		    ...
		    $gasset_list .= '<span class="tooltip_text"  onmousemove="showToolTip(event,\''.htmlentities($gasset_title).'\');return false" onmouseout="hideToolTip()"><img src="'.$gasset_imagesrc.'" style="border:0px" alt="" title="" />&nbsp;</span>';
